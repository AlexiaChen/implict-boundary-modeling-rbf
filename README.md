...
- 在侧边栏勾选 **“Fast (FMM-style sparse solver)”** 可启用近似快速多极子求解路径。
- 该模式使用 KD-Tree 限定 3% 包围盒对角线的邻域半径（最多 96 个邻居），将稠密核矩阵稀疏化后使用 `SparseLU` 求解。
- 当稀疏求解失败时会自动回退到原有的稠密 LU 求解，保证稳健性。
- 对大规模点云可显著降低内存占用，并缩短矩阵构建与求解时间（近似 O(N·k)）。
- 可选 **“Greedy center reduction”** + 上限输入框：使用最远点贪婪采样压缩中心（默认上限 8000），进一步减少矩阵规模。
- 可选 **“Compactly supported RBF”**：使用 Wendland C2 紧凑核（支撑半径默认 5% 包围盒对角线，自动收缩到稀疏半径），核外为 0，从源头获得稀疏性。

## 未来扩展

论文中提到但本基础版本未实现的优化：

1. **更高阶快速多极子（FMM）**：当前实现仅用邻域截断 + 稀疏求解；更高阶 FMM 会做真正的多级分块与多极/局部展开，可将远场贡献压缩到 O(M + N log N)。
2. **贪婪中心缩减**：可用远thest-point/误差驱动的贪婪采样保留代表性中心，预期减少 80-90% 未压缩节点以进一步缩小系统规模。
3. **紧凑支撑 RBF**：切换到 Wendland 等 CSRBF 使核本身稀疏，配合 kd-tree 半径截断可获得稳定的局部解并显著降低内存。
